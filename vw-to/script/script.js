...
function priceHeight() {
  let priceBlocks = $('.price-block');
  let priceBlockHeight = 0;
  priceBlocks.each(function(i, elem) {
    if (priceBlockHeight < $(elem).outerHeight()) {
      priceBlockHeight = $(elem).outerHeight();
    }
  });
  priceBlocks.each(function(i, elem) {
    $(elem).css('height', priceBlockHeight + 'px');
  });
}
// priceHeight();
