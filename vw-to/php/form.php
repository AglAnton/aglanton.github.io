...
<?php
  ...
  $brushes = $_POST["brushes"];//brushes
  $brushesMessage = '';
  if (!empty($brushes)) {
    $brushesMessage = "<li><b>Щётки стеклоочистителя:</b></li>";
    for ($i = 0; $i < count($brushes); $i++) {
      $brushesMessage .= ($i+1 . " - " . $brushes[$i] . "<br>");
    }
    $brushesMessage .= "<br>";
  }
  ...
  if (!empty($name) ||
      ...
      $headers .= "From: <mail@vwto.ru>\r\n"; 
      $headers .= "Reply-To: mail@vwto.ru\r\n";   
